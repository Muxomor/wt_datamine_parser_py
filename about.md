...
#### 5. **misc_and_images_parser.py** - Парсинг дополнительных данных и мультимедиа (НОВЫЙ МОДУЛЬ)
...
#### 6. **main.py** - Точка входа и CLI
**Функции:**
- `main()` - полный парсинг (shop + локализация + wpcost + misc)
...
### Парсер дополнительных данных и мультимедиа (НОВОЕ)
...
### 8. Парсинг дополнительных данных и мультимедиа (misc_and_images_parser.py) (НОВОЕ)
...
- **misc_and_images_parser_debug.log** - полная отладочная информация парсера misc данных (DEBUG уровень) (НОВОЕ)
...
### Примеры логов misc парсера (НОВОЕ)
```
...
### Только парсинг misc данных (НОВОЕ)
```bash
...
- **shop_images_fields.csv** - поля image из shop.blkx для fallback поиска изображений (НОВОЕ)
- **localization.csv** - маппинг ID → русские названия
- **wpcost.csv** - экономические данные (серебро, опыт, БР)
- **rank_requirements.csv** - требования по рангам для открытия новых рангов (НОВОЕ)
- **country_flags.csv** - флаги стран с проверкой доступности (НОВОЕ)
- **shop_images.csv** - изображения техники с двухуровневым поиском (НОВОЕ)
...
- **misc_and_images_parser_debug.log** - подробный лог парсера misc данных с отладкой поиска изображений (НОВОЕ)
- Статистика в консоли включая информацию об удаленных элементах, экономических данных и мультимедиа ресурсах
...
### Расширение отладочных инструментов (НОВОЕ):
...
### Система извлечения image полей (НОВОЕ):
...
Система готова к работе с реальными данными War Thunder и может быть легко адаптирована для фильтрации любого нежелательного контента путем добавления соответствующих префиксов или суффиксов в конфигурационные константы.